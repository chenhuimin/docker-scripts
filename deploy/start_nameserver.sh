...
function start_nameserver() {
    ...
    if [ "$DEBUG" -gt 0 ]; then
        echo docker run -d --name nameserver${DOMAINNAME} -h nameserver${DOMAINNAME} -v $DNSDIR:/etc/dnsmasq.d -p 5353:53/udp $1
    fi
    NAMESERVER=$(docker run -d --name nameserver${DOMAINNAME} -h nameserver${DOMAINNAME} -v $DNSDIR:/etc/dnsmasq.d -p 5353:53/udp $1)
